...
def sample_inside_sphere(dimensionality, radius, p_norm=1):
    """
    :param dimensionality: 生成向量的维度
    :param radius: 对点的半径，要在这个半径内生成点
    :param p_norm: 幂
    :return:
    """
...
class CCR:
    def __init__(self, energy=0.25, cleaning_strategy='translate', selection_strategy='proportional', p_norm=1,
    ...
    def fit_sample(self, X, y):
        ...
        if self.method == 'sampling':
            for i in range(1, len(classes)):
                ...
                used_observations = {}  # 代表本次生成新样本点会使用的数据集
                unused_observations = {}
                ...
                # 因为之前有的样本进行过了改动或者进行了添加(oversampled)，并且有的样本根本没有发生变化（unused_points，
                # 将这两部分进行合并
                for cls in classes:
